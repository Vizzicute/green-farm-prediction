...
const EditPredictionForm = ({ id }: { id: string }) => {
  const trpc = useTRPC();
  const [sport, setSport] = useState("FOOTBALL");

  const { mutate, isPending } = useMutation(
    trpc.adminUpdatePrediction.mutationOptions()
  );
  ...
  const {
    ...
    refetch,
  } = useQuery(trpc.adminGetPredictionById.queryOptions({ id }));
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      tip: prediction.tip,
      subscriptionCategoryId: prediction.subscriptionCategoryId ?? "",
      odd: prediction.odds,
      over: prediction.over,
      chance: prediction.chance,
      htft: prediction.htft,
      either: prediction.either,
      isBanker: prediction.banker,
      isBtts: prediction.btts,
    });
  ...
  if (!isPredictionLoading && !prediction) return notFound();

  if (isPredictionLoading)
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-lg animate-pulse">Loading Edit Prediction Page...</p>
      </div>
    );
  return (
    <Form {...form}>
