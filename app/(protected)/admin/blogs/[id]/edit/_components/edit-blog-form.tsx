...
const EditBlogForm = ({ id }: { id: string }) => {
  ...
  const { mutate, isPending } = useMutation(
    trpc.adminUpdateBlog.mutationOptions()
  );
  ...
  const {
    ...
    refetch,
  } = useQuery(trpc.adminGetBlogPost.queryOptions({ id }));
  ...
  useEffect(() => {
    if (!blogPost || isBlogPostLoading) return;
    ...
    form.reset({
      ...
      categoriesId: blogPost.categories.map((cat) => cat.id),
    });
  }, [blogPost, isBlogPostLoading]);
  ...
  if (!isBlogPostLoading && !blogPost) return notFound();
