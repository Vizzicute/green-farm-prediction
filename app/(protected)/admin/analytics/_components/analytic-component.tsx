"use client";

import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import {
  CreditCard,
  Users,
  TrendingUp,
  DollarSign,
  ArrowUp,
  ArrowDown,
} from "lucide-react";
import { format } from "date-fns";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { useState } from "react";
import AnalyticsSkeleton from "@/app/(protected)/admin/analytics/_components/analytics-skeleton";
import { useTRPC } from "@/lib/trpc/client";
import { useQuery } from "@tanstack/react-query";
import {
  getTimeFilterLabel,
  getDateRange,
  thisMonth,
  lastMonth,
} from "../utils";
import { cn } from "@/lib/utils";

const AnalyticComponent = () => {
  const trpc = useTRPC();
  const [predictionTimeFilter, setPredictionTimeFilter] = useState("all");

  // Subscription stats
  const { data: totalSubscriptions, isLoading: totalSubscriptionLoading } =
    useQuery(trpc.adminGetSubscriptionsCount.queryOptions({ filters: {} }));

  const {
    data: thisMonthSubscriptions,
    isLoading: thisMonthSubscriptionsLoading,
  } = useQuery(
    trpc.adminGetSubscriptionsCount.queryOptions({
      filters: {
        customStartDate: thisMonth.from,
        customEndDate: thisMonth.to,
      },
    })
  );
  const {
    data: lastMonthSubscriptions,
    isLoading: lastMonthSubscriptionsLoading,
  } = useQuery(
    trpc.adminGetSubscriptionsCount.queryOptions({
      filters: {
        customStartDate: lastMonth.from,
        customEndDate: lastMonth.to,
      },
    })
  );

  // User stats
  const { data: totalUsers, isLoading: totalUsersLoading } = useQuery(
    trpc.adminGetUsersCount.queryOptions({ filters: {} })
  );

  const { data: newUsersThisMonth, isLoading: thisMonthUsersLoading } =
    useQuery(
      trpc.adminGetUsersCount.queryOptions({
        filters: {
          role: "user",
          active: true,
          customStartDate: thisMonth.from,
          customEndDate: thisMonth.to,
        },
      })
    );

  const { data: newUsersLastMonth, isLoading: newUsersLastMonthLoading } =
    useQuery(
      trpc.adminGetUsersCount.queryOptions({
        filters: {
          role: "user",
          active: true,
          customStartDate: lastMonth.from,
          customEndDate: lastMonth.to,
        },
      })
    );

  // Blog stats
  const { data: totalBlogs, isLoading: totalBlogsLoading } = useQuery(
    trpc.adminGetBlogPostsCount.queryOptions({ filters: {} })
  );

  const { data: blogsThisMonth, isLoading: blogsThisMonthLoading } = useQuery(
    trpc.adminGetBlogPostsCount.queryOptions({
      filters: {
        customStartDate: thisMonth.from,
        customEndDate: thisMonth.to,
      },
    })
  );

  const { data: blogsLastMonth, isLoading: blogsLastMonthLoading } = useQuery(
    trpc.adminGetBlogPostsCount.queryOptions({
      filters: {
        customStartDate: lastMonth.from,
        customEndDate: lastMonth.to,
      },
    })
  );

  // Prediction stats
  const { from, to } = getDateRange(predictionTimeFilter);
  const { data: totalPredictions, isLoading: totalPredictionsLoading } =
    useQuery(
      trpc.getAdminPredictionsCount.queryOptions({
        filters: {
          customStartDate: from,
          customEndDate: to,
        },
      })
    );

  const { data: winPredictions, isLoading: winPredictionsLoading } = useQuery(
    trpc.getAdminPredictionsCount.queryOptions({
      filters: {
        status: "WON",
        customStartDate: from,
        customEndDate: to,
      },
    })
  );

  const { data: lossPredictions, isLoading: lossPredictionsLoading } = useQuery(
    trpc.getAdminPredictionsCount.queryOptions({
      filters: {
        status: "LOST",
        customStartDate: from,
        customEndDate: to,
      },
    })
  );

  const winRate =
    totalPredictions && winPredictions
      ? ((winPredictions / totalPredictions) * 100).toFixed(1)
      : "0";
  
  if (
    totalSubscriptionLoading ||
    thisMonthSubscriptionsLoading ||
    lastMonthSubscriptionsLoading ||
    totalUsersLoading ||
    thisMonthUsersLoading ||
    totalBlogsLoading ||
    blogsThisMonthLoading
  ) {
    return <AnalyticsSkeleton />;
  }
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold tracking-tight">
          Analytics Dashboard
        </h1>
        <div className="text-sm text-muted-foreground">
          Last updated: {format(new Date(), "PPP")}
        </div>
      </div>

      {/* Overview Cards */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Total Revenue</CardTitle>
            <DollarSign className="h-4 w-4 text-green-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {/* ${totalRevenue.toFixed(2)} */}
              $1,324,204.00
            </div>
            <p className="text-xs text-muted-foreground">
              {/* +${thisMonthRevenue.toFixed(2)} this month */}
              +$103,592.00 this month
            </p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              Total Subscriptions
            </CardTitle>
            <CreditCard className="h-4 w-4 text-blue-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{totalSubscriptions}</div>
            <p className="flex items-center text-xs text-muted-foreground">
              +{thisMonthSubscriptions} this month{" "}
              {thisMonthSubscriptions !== undefined &&
              lastMonthSubscriptions !== undefined &&
              thisMonthSubscriptions !== lastMonthSubscriptions &&
              thisMonthSubscriptions > lastMonthSubscriptions ? (
                <ArrowUp className="size-3 text-green-500" />
              ) : (
                <ArrowDown className="size-3 text-red-500" />
              )}
            </p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Total Users</CardTitle>
            <Users className="h-4 w-4 text-purple-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{totalUsers}</div>
            <p className="flex items-center text-xs text-muted-foreground">
              +{newUsersThisMonth} this month{" "}
              {newUsersThisMonth !== undefined &&
              newUsersLastMonth !== undefined &&
              newUsersThisMonth !== newUsersLastMonth &&
              newUsersThisMonth > newUsersLastMonth ? (
                <ArrowUp className="size-3 text-green-500" />
              ) : (
                <ArrowDown className="size-3 text-red-500" />
              )}
            </p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              Prediction Win Rate
            </CardTitle>
            <TrendingUp className="h-4 w-4 text-orange-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {winRate}%{" "}
              <span className="text-xs font-normal text-muted-foreground">
                {getTimeFilterLabel(predictionTimeFilter)}
              </span>
            </div>
            <p className="text-xs text-muted-foreground">
              {winPredictions} wins / {totalPredictions} total
            </p>
          </CardContent>
        </Card>
      </div>

      {/* Detailed Stats */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {/* Subscription Stats */}
        <Card className="col-span-1">
          <CardHeader>
            <CardTitle>Subscription Overview</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Total</span>
                <span className="text-sm text-blue-500">
                  {totalSubscriptions}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">This Month</span>
                <span className="text-sm text-green-500">
                  {thisMonthSubscriptions}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Last Month</span>
                <span className="text-sm text-cyan-500">
                  {lastMonthSubscriptions}
                </span>
              </div>
            </div>
          </CardContent>
        </Card>

        {/* User Stats */}
        <Card className="col-span-1">
          <CardHeader>
            <CardTitle>User Overview</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Total Users</span>
                <span className="text-sm">{totalUsers}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Last Month</span>
                <span className="text-sm text-cyan-500">
                  {newUsersThisMonth}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">
                  New Users This Month
                </span>
                <span className="text-sm text-green-500">
                  +{newUsersLastMonth}
                </span>
              </div>
            </div>
          </CardContent>
        </Card>

        {/* Blog Stats */}
        <Card className="col-span-1">
          <CardHeader>
            <CardTitle>Blog Overview</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Total Posts</span>
                <span className="text-sm">{totalBlogs}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">This Month</span>
                <span className="text-sm text-green-500">{blogsThisMonth}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Last Month</span>
                <span className="text-sm text-cyan-500">{blogsLastMonth}</span>
              </div>
            </div>
          </CardContent>
        </Card>

        {/* Prediction Stats */}
        <Card className="col-span-1">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle>Prediction Performance</CardTitle>
            <Select
              value={predictionTimeFilter}
              onValueChange={setPredictionTimeFilter}
            >
              <SelectTrigger className="w-[140px]">
                <SelectValue placeholder="Select time" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All Time</SelectItem>
                <SelectItem value="today">Today</SelectItem>
                <SelectItem value="yesterday">Yesterday</SelectItem>
                <SelectItem value="this-week">This Week</SelectItem>
                <SelectItem value="last-week">Last Week</SelectItem>
                <SelectItem value="last-7-days">Last 7 Days</SelectItem>
                <SelectItem value="this-month">This Month</SelectItem>
                <SelectItem value="last-month">Last Month</SelectItem>
              </SelectContent>
            </Select>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Total Predictions</span>
                <span className="text-sm">{totalPredictions}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Wins</span>
                <span className="text-sm text-green-500">{winPredictions}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Losses</span>
                <span className="text-sm text-red-500">{lossPredictions}</span>
              </div>
              <div className="pt-2">
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={cn(
                      "h-2 rounded-full",
                      Number(winRate) >= 50
                        ? "bg-green-500"
                        : Number(winRate) >= 30
                          ? "bg-amber-500"
                          : "bg-red-500"
                    )}
                    style={{ width: `${winRate}%` }}
                  />
                </div>
                <p className="text-xs text-center mt-1">Win Rate: {winRate}%</p>
              </div>
              <div className="text-xs text-muted-foreground text-center">
                Showing stats for {getTimeFilterLabel(predictionTimeFilter)}
              </div>
            </div>
          </CardContent>
        </Card>
      </div>
    </div>
  );
};

export default AnalyticComponent;
