...
import { SocialLinksSettings } from "@/schema/settings";
...
const AdPopupModal = () => {
  ...
  const [link, setLink] = useState("https://t.me/greenfarmprediction");

  // Move useQuery outside useEffect for correct React Query usage
      const { data, isLoading } = useQuery(
        trpc.getSettingsByCategory.queryOptions({ category: "social" })
      );
    
    
      const socials = JSON.parse(data?.value ?? "{}") as SocialLinksSettings;
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/70 z-50">
      <div className="relative bg-white rounded-md text-center">
        ...
        <a href={link} target="_blank" rel="noopener noreferrer">
          <img src="/telegram.jpg" alt="Pop up Advert" className="mx-auto" />
        </a>
      </div>
    </div>
  );
